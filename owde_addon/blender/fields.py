...
from .edge_character import apply_subtle_hand_edge_character
...
def _create_field_base(spec, collection):
    ...
    base.color = (0.016, 0.075, 0.050, 1.0)
...
def _create_field_from_spec(spec, settings=None):
    ...
    if (
        ...
        and settings.edge_character_mode == "SUBTLE_HAND"
        and settings.edge_variation_mm > 0.0
    ):
        apply_subtle_hand_edge_character(
            combined,
            field_top_z_mm=0.0,
            amplitude_mm=settings.edge_variation_mm,
            scale_mm=settings.edge_variation_scale_mm,
            seed=settings.edge_variation_seed,
        )
...
class OWDE_PG_field_settings(PropertyGroup):
    architecture: EnumProperty(
        ...
        items=(
            ...
            ("NESTED_ZONES", "Nested Zones", "FIELD-0005 - nested field zones"),
            ("OFFSET_ZONES", "Offset Zones", "FIELD-0006 - offset field zones"),
            (
                "TERMINAL_TARGET",
                "Terminal / Target",
                "FIELD-0007 - corridor terminating at a target",
            ),
        ),
        default="DIVISION",
    )
    ...
    edge_character_mode: EnumProperty(
        name="Edge Character",
        description=(
            "Control whether field line edges remain exact or receive subtle "
            "hand-like variation"
        ),
        items=(
            ("CLEAN", "Clean", "Exact, CAD-clean architecture edges"),
            (
                "SUBTLE_HAND",
                "Subtle Hand",
                "Apply restrained smooth edge variation while preserving architecture",
            ),
        ),
        default="SUBTLE_HAND",
    )

    edge_variation_mm: FloatProperty(
        name="Variation Amount",
        description="Maximum edge variation in millimetres",
        default=2.0,
        min=0.0,
        max=5.0,
    )

    edge_variation_scale_mm: FloatProperty(
        name="Variation Scale",
        description=(
            "Length scale of the smooth edge variation; larger values produce "
            "broader, slower changes"
        ),
        default=50.0,
        min=5.0,
        max=250.0,
    ...
    edge_variation_seed: IntProperty(
        name="Seed",
...
class OWDE_OT_create_field_architecture(Operator):
    ...
    def execute(self, context):
        ...
        try:
            if settings.architecture == "DIVISION":
            ...
            elif settings.architecture == "DIVERGENCE":
                ...
                raise ValueError(
                    f"Unsupported field architecture: {settings.architecture}"
                )

            base, _line_objects = _create_field_from_spec(
                spec,
                settings=settings,
            )
            bpy.ops.object.select_all(action="DESELECT")
...
class OWDE_PT_field_architectures(Panel):
    ...
    def draw(self, context):
        ...
        edge_box = layout.box()
        edge_box.label(text="Edge Character")
        edge_box.prop(settings, "edge_character_mode")

        if settings.edge_character_mode == "SUBTLE_HAND":
            edge_box.prop(settings, "edge_variation_mm")
            edge_box.prop(settings, "edge_variation_scale_mm")
            edge_box.prop(settings, "edge_variation_seed")
        ...
        if settings.architecture == "DIVISION":
            ...
            architecture_box.label(text="FIELD-0005 - Nested Zones")
            architecture_box.label(text="Centered outer and inner boundaries")
        elif settings.architecture == "OFFSET_ZONES":
            architecture_box.label(text="FIELD-0006 - Offset Zones")
            architecture_box.label(text="Two diagonally displaced zones")
        else:
            architecture_box.label(text="FIELD-0007 - Terminal / Target")
            architecture_box.label(text="Corridor ending at target boundary")
