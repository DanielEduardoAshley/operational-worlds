...
def get_or_create_gallery_collection(
    ...
    if collection is None:
        collection = bpy.data.collections.new(GALLERY_COLLECTION_NAME)
...
class OWDE_OT_create_primitive_gallery(bpy.types.Operator):
    ...
    def execute(self, context: bpy.types.Context):
        ...
        try:
            ...
            self.report({"ERROR"}, str(error))
            return {"CANCELLED"}
        gallery_collection = get_or_create_gallery_collection(context)
        ...
        for index, (parameters, mesh_data) in enumerate(
            ...
            object_name = (
                f"OWDE_Gallery_{index + 1:02d}_"
                f"{parameters.shape.value.title()}"
            )
            object_ = create_blender_object(
                name=object_name,
                mesh_data=mesh_data,
...
class OWDE_OT_frame_primitive_gallery(bpy.types.Operator):
    ...
    bl_description = "Select and frame all objects in the primitive gallery"
    bl_options = {"REGISTER"}
    def execute(self, context: bpy.types.Context):
        ...
        try:
            ...
            self.report(
                {"INFO"},
                "Gallery selected. Press Home or Numpad . to frame it.",
            )
        return {"FINISHED"}
